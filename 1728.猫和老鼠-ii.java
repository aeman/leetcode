            ...
            for (int cat = 0; cat < total; cat++) {
                ...
                for (int[] dir : dirs) {
                    for (int row = mouseRow + dir[0], col = mouseCol + dir[1], jump = 1; row >= 0 && row < rows
                            && col >= 0 && col < cols && grid[row].charAt(col) != '#'
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] prevState : prevStates) {
                ...
                if (results[prevMouse][prevCat][prevTurn][0] == UNKNOWN) {
                    boolean canWin = (result == MOUSE_WIN && prevTurn == MOUSE_TURN)
    ...
    public List<int[]> getPrevStates(int mouse, int cat, int turn) {
        ...
        for (int[] dir : dirs) {
            for (int i = startRow + dir[0], j = startCol + dir[1], jump = 1; i >= 0 && i < rows && j >= 0 && j < cols
                    && grid[i].charAt(j) != '#' && jump <= maxJump; i += dir[0], j += dir[1], jump++) {
